...
def dict_to_tf_example(data,
  ...
  """
  # replacereccoonfilename
  img_path = os.path.join(dataset_directory, data['filename']).replace('png','jpg')
...
if __name__ == '__main__':
  ...
  with io.open(os.path.join(path_home,'config.xml'),'r',encoding='utf8') as f:
      r=f.read()
  soup=bs(r,'xml')
  FLAGS.data_dir=soup.config['image_path']
  FLAGS.set=soup.config['container_train']
  FLAGS.output_path=soup.config['tfrecord_train']
  FLAGS.label_map_path=soup.config['objects_label_map_path']
  FLAGS.annotations_dir=soup.config['image_annotation_path']
  ...
  for year in years:
    ...
    for idx, example in enumerate(examples_list):
      ...
      path = os.path.join(annotations_dir, example + '.xml').replace('\ufeff','')
